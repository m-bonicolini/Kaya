...
#define TAPE_EOF 1
#define TAPE_EOT 0
#define KERNEL_MODE ( (  (((state_t *) 0x20000348)->status) & ~0x00000008) ? 1 : 0)
...
/* Scheduler's pseudo clock tolerance based on the given timescale */
#define PSEUDO_CLOCK_PROXIMITY ( (*((int*)BUS_TIMESCALE)) * 250)
...
#define ENABLE_ALL_INTERRUPTS(status) (status = (status |STATUS_INT_UNMASKED ))
#define DISABLE_ALL_INTERRUPTS(status) (status = (status & ~STATUS_IEc))
#define ENABLE_INTERRUPTS (setSTATUS( getSTATUS()  | STATUS_KER_INT_UNMASKED | STATUS_IEc))

#define DISABLE_INTERRUPTS (setSTATUS( getSTATUS()  & ~STATUS_KER_INT_UNMASKED & ~STATUS_IEc))
#define STATUS_INT_UNMASKED 0x0000ff00
#define STATUS_KER_INT_UNMASKED 0x0000fc00
#define ENABLE_VM (setSTATUS(getSTATUS() | STATUS_VMc | STATUS_VMp | STATUS_VMo))
#define DISABLE_VM (setSTATUS(getSTATUS() & ~STATUS_VMc & ~STATUS_VMp & ~STATUS_VMo))
int dev_sem[4][8];
int dev_term[2][8];
int dev_status[4][8];
int dev_status_term[2][8];
U32 pseudoclock;
int wait4pseudoclock;
